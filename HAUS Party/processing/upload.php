...
$tumblr_email = "xeopauj979@tumblr.com"; // find in your tumblr settings
...
else
{
    ...
    if (strpos($imageURL,"temp3.jpg") > 0 ) {
    	echo "Make gif";
    	
		//header ('Content-type:image/gif');
		include('GIFEncoder.class.php');
		
		$image = imagecreatefromjpeg('saved/bytes/temp1.jpg');
		ob_start();
		imagegif($image);
		...
		$image = imagecreatefromjpeg('saved/bytes/temp2.jpg');
		ob_start();
		imagegif($image);
		$frames[]=ob_get_contents();
		$framed[]=15; // Delay in the animation.
		...
		$image = imagecreatefromjpeg('saved/bytes/temp3.jpg');
		ob_start();
		imagegif($image);
		$frames[]=ob_get_contents();
		$framed[]=15; // Delay in the animation.
		...
		$mail_filename = mktime();
		FWrite ( FOpen ( "gifs/". $mail_filename .".gif", "wb" ), $gif->GetAnimation () );    	
    	
		    	
		$my_file = $mail_filename.".gif";
		$my_path = $_SERVER['DOCUMENT_ROOT']."/processing/gifs/";
		$my_name = "HAUS PARTY";
		$my_mail = "haus_party@madeinhaus.com";
		$my_replyto = "haus_party@madeinhaus.com";
		$my_subject = "HAUS Party: " . $mail_filename;
		$my_message = "HAUS Party: " . $mail_filename;
		//noofliaph773@tumblr.com
		//mail_attachment($my_file, $my_path, "lasse.korsgaard@bbh-usa.com", $my_mail, $my_name, $my_replyto, $my_subject, $my_message);    	
		//mail_attachment($my_file, $my_path, "noofliaph773@tumblr.com", $my_mail, $my_name, $my_replyto, $my_subject, $my_message);    	
		mail_attachment($my_file, $my_path, $tumblr_email, $my_mail, $my_name, $my_replyto, $my_subject, $my_message);    	
//246flewac@tumblr.com    	
    }
